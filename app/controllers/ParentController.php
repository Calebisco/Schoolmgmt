...
class ParentController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'parent';
		$fields = array('parent_id', 'name', 'email', 'password', 'phone', 'address', 'profession', 'authentication_key');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('parent_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Parent";
		$this->view->render('parent/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'parent/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('parent_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Parent";
			$this->view->render('parent/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'parent';
			$fields = $this->fields = array('name','email','password','phone','address','profession','authentication_key'); //insert fields
		...
		$this->view->page_title ="Add New Parent";
		$this->view->render('parent/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'parent';
		$fields = $this->fields = array('parent_id','name','email','password','phone','address','profession','authentication_key'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Parent";
		if(!empty($data)){
			$this->view->render('parent/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
