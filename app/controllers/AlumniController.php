...
class AlumniController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'alumni';
		$fields = array('alumni_id', 'name', 'sex', 'phone', 'email', 'address', 'profession', 'marital_status', 'g_year', 'club', 'interest');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('alumni_id',"%$text%",'LIKE');
			...
			$db->orWhere('profession',"%$text%",'LIKE');
			$db->orWhere('marital_status',"%$text%",'LIKE');
			$db->orWhere('g_year',"%$text%",'LIKE');
			$db->orWhere('club',"%$text%",'LIKE');
			$db->orWhere('interest',"%$text%",'LIKE');
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('alumni_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Alumni";
		$this->view->render('alumni/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'alumni/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('alumni_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Alumni";
			$this->view->render('alumni/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'alumni';
			$fields = $this->fields = array('name','sex','phone','email','address','profession','marital_status','g_year','club','interest'); //insert fields
			...
			$this->sanitize_array = array(
				...
				'g_year' => 'sanitize_string',
				'club' => 'sanitize_string',
		...
		$this->view->page_title ="Add New Alumni";
		$this->view->render('alumni/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'alumni';
		$fields = $this->fields = array('alumni_id','name','sex','phone','email','address','profession','marital_status','g_year','club','interest'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Alumni";
		if(!empty($data)){
			$this->view->render('alumni/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
