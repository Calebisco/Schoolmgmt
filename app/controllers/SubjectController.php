...
class SubjectController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'subject';
		$fields = array('subject_id', 'name', 'class_id', 'teacher_id');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('subject_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->render('subject/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'subject/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if(!empty($record)){
			$this->view->page_title ="View  Subject";
			$this->view->render('subject/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'subject';
			$fields = $this->fields = array('name','class_id','teacher_id'); //insert fields
		...
		$this->view->page_title ="Add New Subject";
		$this->view->render('subject/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'subject';
		$fields = $this->fields = array('subject_id','name','class_id','teacher_id'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		if(!empty($data)){
			$this->view->render('subject/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
