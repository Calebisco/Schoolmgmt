...
class PaymentController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'payment';
		$fields = array('payment_id', 'expense_category_id', 'title', 'payment_type', 'invoice_id', 'student_id', 'method', 'description', 'amount', 'timestamp');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('payment_id',"%$text%",'LIKE');
			$db->orWhere('expense_category_id',"%$text%",'LIKE');
			$db->orWhere('title',"%$text%",'LIKE');
			$db->orWhere('payment_type',"%$text%",'LIKE');
			$db->orWhere('invoice_id',"%$text%",'LIKE');
			$db->orWhere('student_id',"%$text%",'LIKE');
			$db->orWhere('method',"%$text%",'LIKE');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('payment_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Payment";
		$this->view->render('payment/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'payment/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('payment_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Payment";
			$this->view->render('payment/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'payment';
			$fields = $this->fields = array('expense_category_id','title','payment_type','invoice_id','student_id','method','description','amount','timestamp'); //insert fields
			...
			$this->sanitize_array = array(
				'expense_category_id' => 'sanitize_string',
		...
		$this->view->page_title ="Add New Payment";
		$this->view->render('payment/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'payment';
		$fields = $this->fields = array('payment_id','expense_category_id','title','payment_type','invoice_id','student_id','method','description','amount','timestamp'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Payment";
		if(!empty($data)){
			$this->view->render('payment/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
