...
class TeacherController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'teacher';
		$fields = array('teacher_id', 'name', 'birthday', 'sex', 'religion', 'blood_group', 'address', 'phone', 'email', 'password', 'authentication_key');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('teacher_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Teacher";
		$this->view->render('teacher/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'teacher/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('teacher_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Teacher";
			$this->view->render('teacher/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		...
		$this->view->page_title ="Add New Teacher";
		$this->view->render('teacher/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'teacher';
		$fields = $this->fields = array('teacher_id','name','birthday','sex','religion','blood_group','address','phone','email','password','authentication_key'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Teacher";
		if(!empty($data)){
			$this->view->render('teacher/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
