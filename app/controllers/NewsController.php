...
class NewsController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'news';
		$fields = array('news_id', 'news_title', 'date', 'news_content');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('news_id',"%$text%",'LIKE');
			$db->orWhere('news_title',"%$text%",'LIKE');
			$db->orWhere('date',"%$text%",'LIKE');
			$db->orWhere('news_content',"%$text%",'LIKE');
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('news_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->render('news/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'news/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if(!empty($record)){
			$this->view->page_title ="View  News";
			$this->view->render('news/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'news';
			$fields = $this->fields = array('news_title','date','news_content'); //insert fields
			...
			$this->sanitize_array = array(
				'news_title' => 'sanitize_string',
				'date' => 'sanitize_string',
				'news_content' => 'sanitize_string',
			);
		...
		$this->view->page_title ="Add New News";
		$this->view->render('news/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'news';
		$fields = $this->fields = array('news_id','news_title','date','news_content'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  News";
		if(!empty($data)){
			$this->view->render('news/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
