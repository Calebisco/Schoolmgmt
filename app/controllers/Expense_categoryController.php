<?php 
/**
 * Expense_category Page Controller
 * @category  Controller
 */
class Expense_categoryController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'expense_category';
		$fields = array('expense_category_id', 'name');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('expense_category_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Expense Category";
		$this->view->render('expense_category/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'expense_category/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('expense_category_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Expense Category";
			$this->view->render('expense_category/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			if(empty($this->view->page_error)){
				...
				if(!empty($rec_id)){
					set_flash_msg('','');
					redirect_to_page("expense_category");
					return;
				}
				else{
					$page_error = null;
		...
		$this->view->page_title ="Add New Expense Category";
		$this->view->render('expense_category/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'expense_category';
		$fields = $this->fields = array('expense_category_id','name'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Expense Category";
		if(!empty($data)){
			$this->view->render('expense_category/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('expense_category_id' , $rec_id,"=",'OR');
		}
