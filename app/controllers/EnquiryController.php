 ...
 * @category  Controller
 */
class EnquiryController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'enquiry';
		$fields = array('enquiry_id', 'category', 'mobile', 'purpose', 'name', 'whom_to_meet', 'date');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('enquiry_id',"%$text%",'LIKE');
			$db->orWhere('category',"%$text%",'LIKE');
			$db->orWhere('mobile',"%$text%",'LIKE');
			$db->orWhere('purpose',"%$text%",'LIKE');
			$db->orWhere('name',"%$text%",'LIKE');
			$db->orWhere('whom_to_meet',"%$text%",'LIKE');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('enquiry_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Enquiry";
		$this->view->render('enquiry/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'enquiry/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('enquiry_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Enquiry";
			$this->view->render('enquiry/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'enquiry';
			$fields = $this->fields = array('category','mobile','purpose','name','whom_to_meet'); //insert fields
		...
		$this->view->page_title ="Add New Enquiry";
		$this->view->render('enquiry/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'enquiry';
		$fields = $this->fields = array('enquiry_id','category','mobile','purpose','name','whom_to_meet'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Enquiry";
		if(!empty($data)){
			$this->view->render('enquiry/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
