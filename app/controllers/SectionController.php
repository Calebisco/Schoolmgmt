...
class SectionController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'section';
		$fields = array('section_id', 'name', 'nick_name', 'class_id', 'teacher_id');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('section_id',"%$text%",'LIKE');
			$db->orWhere('name',"%$text%",'LIKE');
			$db->orWhere('nick_name',"%$text%",'LIKE');
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('section_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Section";
		$this->view->render('section/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'section/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('section_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Section";
			$this->view->render('section/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'section';
			$fields = $this->fields = array('name','nick_name','class_id','teacher_id'); //insert fields
		...
		$this->view->page_title ="Add New Section";
		$this->view->render('section/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'section';
		$fields = $this->fields = array('section_id','name','nick_name','class_id','teacher_id'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Section";
		if(!empty($data)){
			$this->view->render('section/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
