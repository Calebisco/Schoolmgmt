...
class BookController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'book';
		$fields = array('book_id', 'name', 'description', 'author', 'class_id', 'status', 'price');
		...
		$this->view->page_title ="Book";
		$this->view->render('book/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'book/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('book_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Book";
			$this->view->render('book/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'book';
			$fields = $this->fields = array('name','description','author','class_id','status','price'); //insert fields
		...
		$this->view->page_title ="Add New Book";
		$this->view->render('book/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'book';
		$fields = $this->fields = array('book_id','name','description','author','class_id','status','price'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Book";
		if(!empty($data)){
			$this->view->render('book/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
