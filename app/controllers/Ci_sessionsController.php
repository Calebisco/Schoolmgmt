<?php 
/**
 * Ci_sessions Page Controller
 * @category  Controller
 */
class Ci_sessionsController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$this->view->page_title ="Ci Sessions";
		$this->view->render('ci_sessions/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'ci_sessions/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if(!empty($record)){
			$this->view->page_title ="View  Ci Sessions";
			$this->view->render('ci_sessions/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'ci_sessions';
			$fields = $this->fields = array('id','ip_address','timestamp','data'); //insert fields
		...
		$this->view->page_title ="Add New Ci Sessions";
		$this->view->render('ci_sessions/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'ci_sessions';
		$fields = $this->fields = array('id','ip_address','timestamp','data'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Ci Sessions";
		if(!empty($data)){
			$this->view->render('ci_sessions/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
