...
class Exam_resultController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'exam_result';
		$fields = array('result_id', 'student_id', 'question_id', 'answer');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('result_id',"%$text%",'LIKE');
			$db->orWhere('student_id',"%$text%",'LIKE');
			$db->orWhere('question_id',"%$text%",'LIKE');
			$db->orWhere('answer',"%$text%",'LIKE');
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('result_id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Exam Result";
		$this->view->render('exam_result/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'exam_result/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('result_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Exam Result";
			$this->view->render('exam_result/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'exam_result';
			$fields = $this->fields = array('student_id','question_id','answer'); //insert fields
		...
		$this->view->page_title ="Add New Exam Result";
		$this->view->render('exam_result/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'exam_result';
		$fields = $this->fields = array('result_id','student_id','question_id','answer'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Exam Result";
		if(!empty($data)){
			$this->view->render('exam_result/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
