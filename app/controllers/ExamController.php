...
class ExamController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'exam';
		$fields = array('exam_id', 'name', 'date', 'comment');
		...
		$this->view->page_title ="Exam";
		$this->view->render('exam/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'exam/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('exam_id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Exam";
			$this->view->render('exam/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'exam';
			$fields = $this->fields = array('name','date','comment'); //insert fields
		...
		$this->view->page_title ="Add New Exam";
		$this->view->render('exam/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'exam';
		$fields = $this->fields = array('exam_id','name','date','comment'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Exam";
		if(!empty($data)){
			$this->view->render('exam/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
